...
const renderInvoices = (filterText = '') => {
    ...
    if (filterText) {
        ...
        filteredTransactions = transactions.filter(t => {
            ...
            if (isNumeric) {
                ...
                if (codeMatch && codeMatch[1] === filterLower) return true;

                // If it's a short number (1-3 digits), it's a person code search.
                // Only fallback to txnId matching if they typed 4 or more digits.
                if (filterLower.length >= 4 && txnId.toLowerCase().includes(filterLower)) return true;

                return false;
            }

            return name.toLowerCase().includes(filterLower) ||
                txnId.toLowerCase().includes(filterLower);
        });
    }
    ...
    sortedAsc.forEach(t => {
        ...
        if (t.type.startsWith('payment') || t.type === 'labour_payment') {
            ...
            group.quantities.push(t.quantity + (t.unit ? ' ' + t.unit : ''));
    ...
    const sortedDesc = [...groupedList].sort((a, b) => {
    ...
    sortedDesc.forEach(g => {
        ...
        if (g.type === 'sale') {
            isLeger = (g.items && g.items.length === 1 && g.items[0] === 'Previous Balance');
            displayType = isLeger ? 'leger payment' : 'SALE';
            badgeClass = 'badge-sale';
        }
        if (g.type === 'purchase') displayType = 'PURCHASE';
        if (g.type === 'payment_out') displayType = 'PAYMENT OUT';
        if (g.type === 'payment_in') {
            isLeger = (g.items && g.items.length === 1 && g.items[0] === 'Previous Balance');
            displayType = isLeger ? 'leger payment' : 'PAYMENT IN';
        }
        ...
        const isUrdu = window.currentLang === 'ur';
        const translatedPersonName = getUrduItemName(g.personName, isUrdu);
        const joinedItems = g.items.length > 0 ? g.items.map(i => getUrduItemName(i, isUrdu)).join(', ') : '-';
        const joinedQuantities = isLeger ? '-' : (g.quantities && g.quantities.length > 0 ? g.quantities.join(', ') : '-');

        row.innerHTML = `
            <td class="invoice-cell">
                <div class="cell-primary">${translatedPersonName}</div>
            ...
            <td class="invoice-cell">
                <div class="cell-primary" style="font-weight: 400;">${joinedQuantities}</div>
            </td>
            <td class="invoice-cell">
    ...
    if (renderedCount === 0 && filteredTransactions.length > 0) {
        invoiceEmptyState.classList.remove('hidden');
    }
};
...
if (invoiceDateSearch) {
    invoiceDateSearch.addEventListener('input', () => {
        renderInvoices(invoiceSearch.value.trim());
    });
}
...
window.printInvoice = (txnId, type = 'normal') => {
    ...
    if (isFreight) {
        ...
        itemsHtml = txns.map((t) => {
            ...
            if (t.type === 'sale' || t.type === 'purchase' || t.type === 'labour_charge') {
                itemTotal = (qty * price);
                subtotal += itemTotal;
                ...
                `;
            } else if (t.type.startsWith('payment') || t.type === 'labour_payment') {
                itemTotal = qty * price;
    ...
    let personBalance = 0;
    if (personName && personName.toLowerCase() !== 'cash customer') {
        const nameLower = personName.toLowerCase();
        transactions.forEach(t => {
            if ((t.personName || '').toLowerCase() === nameLower) {
                const total = (t.quantity * t.price) + (parseFloat(t.freight) || 0);
                if (t.type === 'sale') personBalance += total;
                else if (t.type === 'purchase') personBalance -= total;
                else if (t.type === 'payment_out') personBalance += total;
                else if (t.type === 'payment_in') personBalance -= total;
                else if (t.type === 'labour_charge') personBalance -= total;
                else if (t.type === 'labour_payment') personBalance += total;
            }
        });
    }
    ...
    <body>
        <div class="header">
            <div class="company-info">
                <h1>${isUrdu ? 'قریشی سالٹس' : 'Qureshi Salts'}</h1>
            </div>
        ...
        <div class="billing-info">
            <div class="bill-to">
                <h3>${t.billedTo}</h3>
                <p>${getUrduItemName(personName, isUrdu)}</p>
            </div>
        </div>
        ...
        <div class="totals">
            ...
            ` : ''}
            ${(personName.toLowerCase() !== 'cash customer' && personBalance !== 0 && !isFreight && firstTxn.type !== 'sale') ? `
            <div class="total-row final" style="border-top: 1px dashed #333; margin-top: 10px; padding-top: 15px;">
                <span>${isUrdu ? 'بقیہ رقم:' : 'Remaining Balance:'}</span>
                <span style="direction: ltr; display: inline-block;">${formatCurrency(personBalance)}</span>
            </div>
            ` : ''}
        </div>
    ...
    const html = `
        <div style="font-family: ${isUrdu ? "'Noto Nastaliq Urdu', serif" : "'Inter', sans-serif"}; color: #333; direction: ${isUrdu ? 'rtl' : 'ltr'};">
            ...
            <div style="width: 300px; margin-${isUrdu ? 'right' : 'left'}: auto;">
                ...
                ` : ''}
                ${(personName.toLowerCase() !== 'cash customer' && personBalance !== 0 && !isFreight && firstTxn.type !== 'sale') ? `
                <div style="display: flex; justify-content: space-between; border-top: 1px dashed #333; font-weight: bold; font-size: 18px; margin-top: 10px; padding-top: 15px;">
                    <span>${isUrdu ? 'بقیہ رقم:' : 'Remaining Balance:'}</span>
                    <span style="direction: ltr; display: inline-block;">${formatCurrency(personBalance)}</span>
                </div>
                ` : ''}
...
const quickEditCurrentBalance = document.getElementById('quick-edit-current-balance');
const quickEditNewBalance = document.getElementById('quick-edit-new-balance');

let currentEditTxnId = null;

const calculateBalanceForEdit = (personName, excludeTxnId) => {
    ...
    sortedAsc.forEach(t => {
        if ((t.txnId === excludeTxnId || t.id === excludeTxnId) && (t.type.startsWith('payment') || t.type === 'labour_payment')) {
            return;
        }
        if ((t.personName || '').toLowerCase() === nameLower) {
            const total = (t.quantity * t.price) + (parseFloat(t.freight) || 0);
...
const updateQuickEditNewBalance = () => {
    const currentBal = parseFloat(quickEditCurrentBalance.value.replace(/[^0-9.-]+/g, "")) || 0;
    const amountPaid = parseFloat(quickEditAmount.value) || 0;

    const txns = transactions.filter(t => t.txnId === currentEditTxnId || t.id === currentEditTxnId);
    if (!txns.length) return;
    const firstTxn = txns.find(t => !t.type.includes('payment')) || txns[0];

    let newBal = currentBal;
    if (firstTxn.type === 'sale' || firstTxn.type === 'payment_in') {
        newBal = currentBal - amountPaid;
    } else if (firstTxn.type === 'purchase' || firstTxn.type === 'payment_out') {
        newBal = currentBal + amountPaid;
    } else if (firstTxn.type === 'labour_charge' || firstTxn.type === 'labour_payment') {
        newBal = currentBal + amountPaid;
    }

    quickEditNewBalance.value = formatCurrency(newBal);
};
...
if (editQuickBtn) {
    editQuickBtn.addEventListener('click', () => {
        ...
        if (currentEditTxnId) {
            ...
            const currentBal = calculateBalanceForEdit(firstTxn.personName, currentEditTxnId);
            quickEditCurrentBalance.value = formatCurrency(currentBal);
            updateQuickEditNewBalance();
...
if (quickEditAmount) {
    quickEditAmount.addEventListener('input', updateQuickEditNewBalance);
}
